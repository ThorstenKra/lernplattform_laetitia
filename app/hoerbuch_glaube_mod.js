...
(function(){
  ...
  // ── Lautstärke ──────────────────────────────────────────────────────────────
  // Lautstaerke: zentral ueber geraete.js
  var _geraete = window.LaetitiaGeraete || null;
  function ladeVolumen(){
    return _geraete ? _geraete.ladeVolumen() : 0.6;
  }
  function bindVolButtons(){
    if(!_geraete) return;
    _geraete.initLautstaerke({
      btnPlus:  "btnVolPlus",
      btnMinus: "btnVolMinus",
      anzeige:  "volAnzeige",
      onAendern: function(v){
  ...
  function bindReturnButtons(){
    ...
    if(btnMain) btnMain.addEventListener("click", function(ev){
      ...
      if(state.screen === "player"){
        ...
        stopAudio();
        window.location.href = "../../../glaube.html";
      }
    });
  }
