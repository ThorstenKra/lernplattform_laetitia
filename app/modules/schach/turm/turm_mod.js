// turm_mod.js -- Laetitia Lernsystem
...
// ── Level-Definitionen ────────────────────────────────────────────────────
const LEVELS = [
  // ── GRUPPE 1: Gerade Linien verstehen ──
  {
    id:1, group:"Turm lernen", name:"Erste Linie",
    boardSize:4, playerStart:[3,0], goal:[0,0], blocks:[], enemy:null, moveLimit:1,
    tutorial:{
      title:"Der Turm – wie er zieht",
      text:"Der Turm zieht in geraden Linien: nach oben, unten, links oder rechts — so weit er will! Er kann nicht diagonal ziehen.",
      tip:"♖ = Turm  |  ⭐ = Ziel  |  Blaue Felder = mögliche Züge"
    }
  },
  {
    id:2, group:"Turm lernen", name:"Eine Reihe",
    boardSize:4, playerStart:[3,0], goal:[3,3], blocks:[], enemy:null, moveLimit:1,
    tutorial:null
  },
  {
    id:3, group:"Turm lernen", name:"Zwei Züge",
    boardSize:4, playerStart:[3,0], goal:[0,3], blocks:[], enemy:null, moveLimit:2,
    tutorial:{
      title:"Zwei Züge reichen!",
      text:"Der Turm braucht von einer Ecke zur anderen nur zwei Züge: einmal nach oben, einmal nach rechts.",
      tip:"Erst Spalte oder erst Reihe — beides funktioniert!"
    }
  },
  {
    id:4, group:"Turm lernen", name:"Kreuz und quer",
    boardSize:5, playerStart:[4,2], goal:[0,2], blocks:[], enemy:null, moveLimit:1,
    tutorial:null
  },
  {
    id:5, group:"Turm lernen", name:"Zickzack",
    boardSize:5, playerStart:[4,0], goal:[0,4], blocks:[], enemy:null, moveLimit:2,
    tutorial:null
  },

  // ── GRUPPE 2: Hindernisse umfahren ──
  {
    id:6, group:"Hindernisse", name:"Erster Block",
    boardSize:4, playerStart:[3,0], goal:[0,0], blocks:[[2,0]], enemy:null, moveLimit:3,
    tutorial:{
      title:"Hindernisse ausweichen",
      text:"Blöcke stoppen den Turm! Er kann nicht über sie hinweg ziehen. Fahre drum herum: in eine andere Reihe oder Spalte wechseln.",
      tip:"■ = Block — der Turm muss davor stoppen!"
    }
  },
  {
    id:7, group:"Hindernisse", name:"Zwei Blöcke",
    boardSize:4, playerStart:[3,0], goal:[0,3], blocks:[[3,2],[1,0]], enemy:null, moveLimit:4,
    tutorial:null
  },
  {
    id:8, group:"Hindernisse", name:"Engpass",
    boardSize:5, playerStart:[4,0], goal:[0,4], blocks:[[2,1],[2,2],[2,3]], enemy:null, moveLimit:4,
    tutorial:{
      title:"Engpass",
      text:"Manchmal gibt es nur eine Lücke. Finde den einzigen freien Weg durch die Blockade!",
      tip:"Suche die Lücke in der Blockade!"
    }
  },
  {
    id:9, group:"Hindernisse", name:"Labyrinth",
    boardSize:5, playerStart:[4,0], goal:[0,4],
    blocks:[[3,1],[3,3],[1,1],[1,3],[2,2]], enemy:null, moveLimit:6,
    tutorial:null
  },

  // ── GRUPPE 3: Planung ──
  {
    id:10, group:"Planung", name:"Kürzester Weg",
    boardSize:5, playerStart:[4,4], goal:[0,0], blocks:[], enemy:null, moveLimit:2,
    tutorial:{
      title:"Immer nur 2 Züge!",
      text:"Mit dem Turm kommt man von jeder Ecke zu jeder anderen Ecke in genau 2 Zügen. Das ist sein großer Vorteil!",
      tip:"Erst Reihe, dann Spalte — oder umgekehrt."
    }
  },
  {
    id:11, group:"Planung", name:"Umwege",
    boardSize:5, playerStart:[4,0], goal:[0,4],
    blocks:[[3,0],[0,1],[0,2],[0,3]], enemy:null, moveLimit:4,
    tutorial:null
  },
  {
    id:12, group:"Planung", name:"Drei Züge",
    boardSize:5, playerStart:[2,2], goal:[0,4],
    blocks:[[2,3],[2,4],[1,4],[0,0],[0,1],[0,2],[0,3]], enemy:null, moveLimit:3,
    tutorial:null
  },
  ...
  // ── GRUPPE 4: Großes Brett ──
  {
    ...
    boardSize:6, playerStart:[5,0], goal:[0,5], blocks:[], enemy:null, moveLimit:2,
    tutorial:{
      title:"Noch mehr Platz!",
      text:"Auf einem größeren Brett hat der Turm noch mehr Kraft — er zieht genauso weit wie er will.",
      tip:"Denke in Linien: erst Reihe oder erst Spalte?"
    }
  },
  {
    id:14, group:"Großes Brett", name:"6×6 mit Blöcken",
    boardSize:6, playerStart:[5,0], goal:[0,5],
    blocks:[[4,1],[3,2],[2,3],[1,4]], enemy:null, moveLimit:6,
    tutorial:null
  },
  {
    id:15, group:"Großes Brett", name:"Finale",
    boardSize:6, playerStart:[5,0], goal:[0,5],
    blocks:[[4,0],[4,2],[2,0],[2,2],[2,4],[0,1],[0,3]], enemy:null, moveLimit:6,
    tutorial:{
      title:"Das große Finale!",
      text:"Das schwierigste Labyrinth. Plane jeden Zug genau vor!",
      tip:"Suche den freien Weg — es gibt einen!"
    }
  }
];
...
function rebindDwell(noProtect){
  // Zellen: data-pdwell entfernen (werden neu erstellt)
  // Buttons: data-pdwell entfernen für statische Buttons
...
// Turm-Zugberechnung: alle erreichbaren Felder in 4 Richtungen
function legalMoves(fromPos, n, blocksSet){
  ...
  dirs.forEach(([dr,dc])=>{
    ...
    while(inBounds(r,c,n)){
      if(blocksSet.has(`${r},${c}`)) break; // Block stoppt
      moves.push([r,c]);
      r += dr;
      c += dc;
    }
...
function showTutorial(tut, onOk){ onOk(); } // Tutorial deaktiviert — direkt zum Level
...
function render(){
  ...
  viewRoot.innerHTML = `
    ...
    <div class="legend">
      <div class="pill blau"><b>▪</b> Zug möglich</div>
      <div class="pill blau"><b>♖</b> Turm</div>
    ...
    <div class="btn-gruppe">
      <button class="btn blau" id="btnNextLevel" ${game.status==="won"?"":"disabled"}>Nächstes ▶</button>
  ...
  for(let r=0; r<game.n; r++){
    for(let c=0; c<game.n; c++){
      ...
      // Felder auf derselben Linie wie der Turm (visueller Hinweis)
      const sameLinie = !isBlock && !isPlayer &&
        (r===game.player[0] || c===game.player[1]);
      ...
      else if(sameLinie && !isGoal) cls += " linie";
      else          cls += " inactive";
...
function showHint(){
  const hints = [
    "Der Turm zieht in geraden Linien — nie diagonal!",
    "Wechsle zuerst in die richtige Reihe oder Spalte.",
    "Blaue Felder zeigen alle möglichen Züge.",
    "Blöcke stoppen den Turm — fahre drum herum.",
    "Von Ecke zu Ecke: nur 2 Züge nötig!"
  ];
