// christlichelieder_mod.js -- Laetitia Lernsystem
...
(function(){
  ...
  var alben      = window.LaetitiaChristlicheLieder || [];
  var screen     = "alben";
  var aktAlbum   = null;
  var aktIdx     = 0;
  var audio      = null;
    ...
    alben.forEach(function(alb, i){
      ...
      a.innerHTML =
        ...
        '<div class="album-kachel-info">' +
          ...
          '<div class="album-kachel-anzahl">' + (alb.christlichelieder?alb.christlichelieder.length:0) + ' Christliche Lieder</div>' +
        '</div>';
  ...
  function zeigeAlbum(alb){
    ...
    elListeAnzahl.textContent = alb.christlichelieder.length + " Christliche Lieder";
    elPsalmListe.innerHTML = "";
    alb.christlichelieder.forEach(function(ps, i){
      var a = document.createElement("a");
  ...
  function spielePsalm(idx){
    if(!aktAlbum) return;
    aktIdx = idx;
    var ps = aktAlbum.christlichelieder[idx];
    if(!ps) return;
    ...
    audio.addEventListener("ended", function(){
      if(aktIdx < aktAlbum.christlichelieder.length - 1){
    ...
    btnNaechst.setAttribute("aria-disabled", idx===aktAlbum.christlichelieder.length-1 ? "true" : "false");
  ...
  btnNaechst.addEventListener("click", function(e){
    e.preventDefault();
    if(aktAlbum && aktIdx<aktAlbum.christlichelieder.length-1) spielePsalm(aktIdx+1);
  });
