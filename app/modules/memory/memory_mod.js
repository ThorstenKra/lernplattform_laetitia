// app/modules/memory/memory_mod.js  v8
...
// NEU v8: Vogel-Set mit echten Bildpaaren (unterschiedliche Bilder pro Paar), eigene Stufen V1–V5
//
// 🎵 MUSIK-DATEI:
...
// ══════════════════════════════════════════════════════════════════════════════
// SET-TYPEN
// Ein Set hat entweder type:"emoji" (bisheriges System)
// oder type:"bild" (echte Bildpaare — unterschiedliche Bilder pro Paar)
// ══════════════════════════════════════════════════════════════════════════════

// Emoji-Sets: jede Karte zeigt dasselbe Emoji wie ihr Paar
const SYMBOL_SETS = {
  tiere:   { id:"tiere",   type:"emoji", label:"Tiere",   emoji:["🐶","🐱","🐰","🦊","🐻","🐼","🐨","🐯","🐸","🐧","🦋","🐬"] },
  essen:   { id:"essen",   type:"emoji", label:"Essen",   emoji:["🍕","🍔","🍦","🍩","🍪","🎂","🍓","🍉","🍌","🍎","🥕","🌽"] },
  objekte: { id:"objekte", type:"emoji", label:"Objekte", emoji:["⭐","🌈","🚀","🎈","🎁","🔑","💎","🏆","🎯","🎸","🚗","🔮"] },

  // Bild-Set: Marjolein Bastin — Zauberhaftes Vogel-Memo (Coppenrath)
  // Dateien: app/modules/memory/images/vogel/VOGELNAME_a.webp + _b.webp
  // 🎙️ MARKER: TTS-Name beim Aufdecken → speak(card.name) optional
  voegel: {
    id:"voegel", type:"bild", label:"Vögel 🐦",
    imgBase: "./images/vogel/",
    paare: [
      { id:"bachstelze",     name:"Bachstelze"     },
      { id:"blaumeise",      name:"Blaumeise"       },
      { id:"buntspecht",     name:"Buntspecht"      },
      { id:"dompfaff",       name:"Dompfaff"        },
      { id:"eisvogel",       name:"Eisvogel"        },
      { id:"elster",         name:"Elster"          },
      { id:"gartenrotschwanz",name:"Gartenrotschwanz"},
      { id:"goldhaehnchen",  name:"Goldhähnchen"    },
      { id:"haussperling",   name:"Haussperling"    },
      { id:"kleiber",        name:"Kleiber"         },
      { id:"kohlmeise",      name:"Kohlmeise"       },
      { id:"rotkehlchen",    name:"Rotkehlchen"     },
      { id:"schwalbe",       name:"Schwalbe"        },
      { id:"schwanzmeise",   name:"Schwanzmeise"    },
      { id:"schwarzspecht",  name:"Schwarzspecht"   },
      { id:"star",           name:"Star"            },
      { id:"stieglitz",      name:"Stieglitz"       },
      { id:"zaunkoenig",     name:"Zaunkönig"       },
    ]
  }
};

// Standard-Stufen für Emoji-Sets (S1–S5, max 12 Paare)
const STUFEN_EMOJI = [
...
// Vogel-Stufen (V1–V5, max 18 Paare — alle Vögel spielbar)
const STUFEN_VOEGEL = [
  { id:"V1", label:"Stufe 1", paare:3,  cols:3, rows:2, beschreibung:"6 Karten · sehr einfach"   },
  { id:"V2", label:"Stufe 2", paare:6,  cols:4, rows:3, beschreibung:"12 Karten · einfach"       },
  { id:"V3", label:"Stufe 3", paare:9,  cols:6, rows:3, beschreibung:"18 Karten · mittel"         },
  { id:"V4", label:"Stufe 4", paare:12, cols:6, rows:4, beschreibung:"24 Karten · schwer"         },
  { id:"V5", label:"Stufe 5", paare:18, cols:6, rows:6, beschreibung:"36 Karten · alle Vögel"     }
];

// Aktuelle Stufen-Liste — wechselt je nach aktivem Set
function getStufen(){
  return activeSet.type === "bild" ? STUFEN_VOEGEL : STUFEN_EMOJI;
}

// Alle Sets zusammengeführt für UI
const ALL_SETS = Object.values(SYMBOL_SETS);
  ...
  _dwellHandle = attach(selector, {
    ...
    onActivate: (el) => {
      ...
      if(btn.classList.contains("stufeBtn")){
        const stufe = getStufen().find(s => s.id === btn.dataset.stufeId);
        if(stufe) startGame(stufe);
      } else if(btn.classList.contains("setBtn")){
        const set = ALL_SETS.find(s => s.id === btn.dataset.setId);
        if(set){
          activeSet = set;
...
function renderSetButtons(){
  ...
  ALL_SETS.forEach(set=>{
    ...
    if(set.type === "bild"){
      // Vorschau: erste 3 Bilder als Mini-Thumbnails
      const imgs = set.paare.slice(0,3).map(p=>
        `<img src="${set.imgBase}${p.id}_a.webp" alt="${p.name}"
              style="width:28px;height:36px;object-fit:cover;border-radius:4px;">`
      ).join("");
      btn.innerHTML = `<span class="setEmoji" style="display:flex;gap:3px;align-items:center;">${imgs}</span><span class="setLabel">${set.label}</span>`;
    } else {
...
function renderStufeButtons(){
  ...
  getStufen().forEach(stufe=>{
    ...
    let preview = "";
    if(activeSet.type === "bild"){
      // Vorschau: 3 Vogelbilder als kleine Thumbnails
      preview = activeSet.paare.slice(0,3).map(p=>
        `<img src="${activeSet.imgBase}${p.id}_a.webp" alt="${p.name}"
              style="width:26px;height:32px;object-fit:cover;border-radius:3px;">`
      ).join("");
      preview = `<div class="stufePreview" style="display:flex;gap:3px;justify-content:center;">${preview}</div>`;
    } else {
      const emojis = activeSet.emoji.slice(0,Math.min(3,stufe.paare)).join(" ");
      preview = `<div class="stufePreview">${emojis}</div>`;
    }
    btn.innerHTML = `
...
function startGame(stufe){
  ...
  // Karten erzeugen — je nach Set-Typ unterschiedlich
  if(activeSet.type === "bild"){
    // Bild-Set: Paare aus der Paare-Liste, gemischt, zufällig N davon
    const pool = shuffle([...activeSet.paare]);
    const selected = pool.slice(0, stufe.paare);
    // Jedes Paar ergibt zwei Karten mit paarId aber unterschiedlichen Bildern
    const flat = [];
    selected.forEach(paar => {
      flat.push({ paarId: paar.id, name: paar.name, seite:"a",
                  src: activeSet.imgBase + paar.id + "_a.webp" });
      flat.push({ paarId: paar.id, name: paar.name, seite:"b",
                  src: activeSet.imgBase + paar.id + "_b.webp" });
    });
    cards = shuffle(flat).map((c,i) => ({ ...c, id:i, matched:false, flipped:false }));
  } else {
    // Emoji-Set: klassisch — zwei identische Emojis pro Paar
    const emojis   = activeSet.emoji.slice(0, stufe.paare);
    const shuffled = shuffle([...emojis, ...emojis]);
    cards = shuffled.map((emoji,i)=>({ id:i, emoji, paarId:emoji, matched:false, flipped:false }));
  }
...
function renderBoard(stufe){
  ...
  cards.forEach((card,i)=>{
    ...
    // Rückseite (immer gleich)
    const back = document.createElement("div");
    back.className = "cardFace cardBack";
    back.innerHTML = `<span class="cardSymbol">❓</span>`;

    // Vorderseite — Emoji oder Bild
    const front = document.createElement("div");
    front.className = "cardFace cardFront";

    if(card.src){
      // Bild-Karte mit Name darunter
      front.style.flexDirection = "column";
      front.style.padding = "4px";
      front.innerHTML = `
        <img src="${card.src}" alt="${card.name}"
             style="flex:1;min-height:0;width:100%;object-fit:contain;border-radius:8px;"
             onerror="this.style.display='none';this.nextElementSibling.style.fontSize='clamp(18px,3vw,32px)'">
        <div style="font-size:clamp(9px,1.4vw,14px);font-weight:900;text-align:center;
                    padding:2px 2px 0;line-height:1.1;color:#0f172a;
                    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
                    width:100%;">${card.name}</div>`;
    } else {
      // Emoji-Karte
...
function onCardClick(idx){
  ...
  // Paar-Vergleich: bei Bild-Sets über paarId, bei Emoji-Sets über paarId (= emoji-Wert)
  const isMatch = first.paarId === card.paarId && first.id !== card.id;
  if(isMatch){
    // ✅ PAAR GEFUNDEN
