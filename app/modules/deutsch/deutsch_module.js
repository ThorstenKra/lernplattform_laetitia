(function(){ // VERSION: 2026-03-15-FINAL
  ...
  async function loadTasks(){
    // 1) Datei-Protokoll-sicher (Registry)
    if(DR){
      const v = DR.get("deutsch");
      if(Array.isArray(v) && v.length) return v;
    }
    // 2) Fallback: fetch (wenn du per локalem Server testest)
    const res = await fetch("./data/deutsch_aufgaben.json", {cache:"no-store"});
    if(!res.ok) throw new Error("deutsch_aufgaben.json konnte nicht geladen werden");
    const json = await res.json();
    const all = Array.isArray(json.tasks) ? json.tasks : [];
    const okLevels = new Set(["A1","A2","A3","A4","A5","A6","A7","A8","L1","L2","L3","L4","L5","L6","L7","L8"]);
    return all.filter(t => okLevels.has(String(t.stufe||"").toUpperCase()));
  }
  ...
  function answer(letter){
    ...
    // letter = Slot-Position (A/B/C/D = Button 1/2/3/4)
    // currentShuffleMap[letter] = die original-Antwort die in diesem Slot liegt
    ...
    try{
      if(window.LaetitiaStats){
        window.LaetitiaStats.taskAnswer(
          taskId(t),
          correct,
          upper(letter),
          false,
  ...
  function bindUI(){
    ...
    // ── Hilfe-Button → eigene Hilfe-Seite (analog Lesemodul) ─────────────
    $("btnHelp")?.addEventListener("click",(ev)=>{
      ev.preventDefault();
      const t = current[index];
      if(!t) return;
      // Stats: Hilfe-Aufruf markieren
      try{
