...
Claude liest beide Dateien, bestätigt die 20 Goldstandard-Regeln und nennt offene Aufgaben.
...
## Sitzungsprotokoll 2. August 2026 — Sitzung 19

| Was | Ergebnis |
|---|---|
| Neue Goldstandard-Regel 20 formalisiert | ✅ „Antwortvorschlag ≠ Auslösefeld" — bei jeder Mehrfachauswahl (auch einzelne Wörter) muss der Optionstext zuerst passiv lesbar sein (`pointer-events:none`), getrennt vom Dwell-Auslösebutton. Vorher nur implizit in der Grammatik-Werkstatt umgesetzt (Sitzung 12), nie projektweit formalisiert. Vollständiges Muster (moduleKit `read1txt`–`read4txt`, `leseHtml()`-`extraHtml`-Parameter) in PROJEKT_WISSEN.md dokumentiert (commit `016b43e`) |
| Projektweiter Regel-20-Audit + Fix | ✅ Verstöße in 9 Dateien gefunden und behoben: Reimen-Werkstatt (`reim_wahl`/`luecke_wahl` zeigten Reimwörter nur auf den Buttons), alle vier KI-Agenten-Chats (Nova/Fabu/Milo/Gruppenchat — Antwortvorschläge nur auf `vorschlag-btn`), `schule_raetsel`, `schule_mathe`, `schule_buchstaben`, `englisch` (Wort-Aufgabe). Grammatik/Lesen/Deutsch/`schule_lesen` bereits konform (nutzen die Muster bzw. Text-links/Button-rechts-Goldstandard). Live per CDP gegen die deployte OneDrive-Version verifiziert (gefälschte TTS/XHR-Antworten, kein Warten auf echten Gemini-Key nötig), kompletter Antwort-Flow (Reimen) ohne Regression, keine Konsolenfehler (commit `016b43e`) |
| Nova/Milo: 5 zusätzliche Eröffnungsgesprächsvarianten (c–g) | ✅ Bisher nur 2 tagesabhängig wechselnde Varianten (a/b) je Charakter. Neu je Charakter fünf weitere: Rückblick-Anknüpfung ans letzte Gespräch, Tier-/Wetterbeobachtung, direkte Tagesfrage bzw. Was-wäre-wenn/Musik-Einstieg, sowie eine Lieblingsfrage-Variante (Lieblingsmensch/-tier/-film/-farbe/-essen, wechselnd, immer nur eine Frage). `listener.ps1`: Auswahllogik von starrem a/b-Tagesparitäts-Check auf generische Auswahl über beliebig viele Varianten (Tag-des-Jahres modulo Anzahl, alphabetisch sortiert) umgestellt — funktional getestet. **Wichtig beim Deploy:** die OneDrive-Kopie von `listener.ps1` enthält den echten Gemini-Key — nur der betroffene Code-Block wurde chirurgisch gepatcht (nicht die ganze Datei überschrieben), Diff zeigte danach nur noch den erwarteten Key-Unterschied. Live per curl für alle vier Agenten verifiziert, inkl. Gruppenchat-Eröffnung über Nova (commit `94dbf59`) |
| Einheitliche Sprechgeschwindigkeit für alle KI-Agenten | ✅ Milo (0.92), Fabu (0.88) und Gruppenchat (0.92) hatten unterschiedliche TTS-Raten, Nova (1.104, `RATE_BASIS`) war die schnellste. Alle drei jetzt auf 1.104 angeglichen — Novas eigene, nutzerseitig einstellbare Tempo-Funktion (50–150%) blieb unverändert. Live in Edge per abgefangener `SpeechSynthesisUtterance.rate` für alle vier Agenten bestätigt (jeweils exakt 1.104) (commit `79289b8`) |
| Bildschirmtastatur („Eigene Antwort") an NuVoice-Vorlage angepasst | ✅ Betrifft Milo, Nova, Gruppenchat (Fabu hat keine freie Texteingabe). Auf Nutzer-Screenshot hin: Zahlenreihe (1–9, 0) ergänzt (konnte bisher keine Zahlen eingeben), echte Groß-/Kleinschreibung eingebaut („Groß" dauerhaft wie Caps Lock + „1× Groß" nur nächster Buchstabe — vorher wurden Buchstaben nur groß angezeigt, aber immer klein getippt, Anzeige und Eingabe stimmten nicht überein), Buchstaben-Reihenfolge an echtes QWERTZ angeglichen (ß am Ende Reihe 2, ä ö ü am Ende Reihe 3), Satzzeichen `–` und `?` ergänzt. Optik: Schriftgröße 18px→28px, Ränder 2px→3px in dunklem Ton (#1e293b) für den markanten Kachel-Look aus dem Referenzscreenshot. Live in Edge für alle drei Agenten funktional (45 Tasten, Klein-Standard/1×Groß/Dauerhaft-Groß-Umschaltung korrekt) und per Screenshot visuell bestätigt — auch nach Listener-Neustart erneut gegengetestet (commit `0f53939`) |
| Nova: dezenter violetter Bildschirmhintergrund | ✅ Erster Schritt einer „Charakter-Welten"-Idee (Nutzerwunsch: jede Agenten-Welt soll unbewusst atmosphärisch spürbar sein, nicht nur über Akzentfarbe an Buttons). Dabei entdeckt: Milo (`#f0fdfa`, türkis) und Fabu (`#fffbeb`, creme) hatten diesen ganz schwachen Bildschirmhintergrund bereits, nur Nova nutzte neutrales Grau (`#f1f5f9`) — jetzt auf `#f8f6fe` (Violett) umgestellt, damit alle drei Welten konsistent atmosphärisch getönt sind. Live in Edge per Screenshot bestätigt (Kontrast/Lesbarkeit unverändert, da nur der reine Bildschirm-Hintergrund betroffen ist). Motiv-Ergänzung (Sterne/Eule/Blätter als dezente Muster) auf Nutzerwunsch auf später verschoben (commit `ac9ad3a`) |

**Commits (alle gepusht):** `016b43e`, `94dbf59`, `79289b8`, `0f53939`, `ac9ad3a`

**Sitzungsabschluss:** Alle fünf Commits gepusht, nach jedem Schritt nach OneDrive deployed und gepinnt, `validate.ps1` durchgehend 0 Fehler. Git-Arbeitsverzeichnis sauber. Live-Tests ausschließlich per eigener Edge-CDP-Testinstanz (`--remote-debugging-port=9222`, separates Profil `C:\EdgeDwellTest`) — die Chrome-Erweiterung war diese Sitzung durchgehend nicht verbunden, Testinstanz nach jedem Durchlauf sauber beendet (nur der eigene Hauptprozess, nie die regulären Edge-Fenster des Nutzers). Listener-Task (`Laetitia_Nova_Listener`) mehrfach auf Nutzerwunsch neu gestartet, jeweils per curl funktional bestätigt.
