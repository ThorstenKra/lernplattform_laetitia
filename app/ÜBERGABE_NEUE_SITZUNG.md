...
*Stand: 1. August 2026 (Sitzung 18 — abgeschlossen). Wichtigster offener Punkt für die nächste Sitzung: **Gemini-API-Key fehlt in der laufenden `listener.ps1`-Instanz** (Platzhalter statt echtem Key, betrifft ALLE Online-Agenten-Features — Nova, Fabu, Milo) — siehe Abschnitt „🔴 Offene Aufgaben — Hochpriorität" weiter unten, braucht den Nutzer. Zweitwichtigster Punkt: Fabu-Stimme (siehe Abschnitt „🔴 Fabu-Stimme"). Sehr umfangreiche Sitzung in vier Teilen: (1) ausstehenden Commit aus Sitzung 17 nachgeholt, danach Mathe-Modul-Ausbau Schritt 4 (4-Stufen-Navigation) umgesetzt — Mathe-Ausbau-Vorhaben (Schritte 1–4) damit abgeschlossen; (2) auf Nutzerwunsch Logik-Modul analysiert, zwei Bugs behoben (Header, fehlende Statistik-Sichtbarkeit) und Milo-Anschluss Stufe A+B umgesetzt (offline Sokratischer Hinweis-Button + Milo-Chat-Kontext + Online-Anfrage mit robustem Fallback — dabei den API-Key-Fund gemacht); (3) Logik-Modul um 4 neue Stufen L6–L9 erweitert (48 neue Aufgaben, 66→114 gesamt), dabei einen echten Bug vor dem Commit gefunden und behoben (kollidierende Aufgaben-IDs bei L8); (4) Mathe-Modul um Rechenweg-Visuals für M1–M4 ergänzt (größte didaktische Lücke aus der Analyse geschlossen — Addition/Subtraktion jetzt visuell wie bisher nur M0/Zählen); (5) zwei kleinere Optik-Funde final behoben — unstyled Glossar-Hilfe-Overlay (Mathe + Sinnesorgane-Quiz) und kollabierte Trennleerzeichen bei Logik-Musteraufgaben (betraf auch L1); (6) Sichtbarkeits-Entscheidung `schule_mathe` getroffen — eigener Statistik-Tab + Milo-Kontext, wie schon bei Mathe/Logik. Damit sind alle Vorschläge aus beiden Analysen abgearbeitet, nur zwei größere unbeauftragte Einzelideen bleiben offen (Fabu-Rechengeschichten, visuelle Matrizen-Aufgaben). Siehe Abschnitte „Mathe-Modul-Ausbau", „Logik-Modul: Analyse + Ausbau" und „Sitzungsprotokoll Sitzung 18" weiter unten. Live überwiegend in Chrome getestet (Browser-Erweiterung war diese Sitzung verbunden), dabei mehrere kleine Bugs gefunden und sofort behoben.*
...
1. Fabu-Rechengeschichten (optional, größerer Aufwand — Matheaufgaben als Erzählungen, analog zur Gedichte-Integration).
...
**Ist-Zustand (Stand nach Content-Ausbau, siehe unten):** 9 Stufen (L1 Was passt nicht?/L2 Muster/L3 Analogien/L4 Richtig-Falsch/L5 Wenn-Dann/L6 Reihenfolgen/L7 Ursache-Wirkung/L8 Widersprüche/L9 Mengenvergleiche), 114 Aufgaben, alle mit `erklaerung`-Feld (Mathe brauchte dafür noch einen eigenen Schritt), Lese-/Aktionsbereich-Trennung (Regel 17) schon korrekt. Die 4-Stufen-Navigation von Mathe wurde als **nicht übertragbar** bewertet — bei ursprünglich nur 5 (jetzt 9) Stufen passt die flache Liste ohne Scrollen weiterhin gut, eine zusätzliche Kachel-Ebene wäre unnötiger Klick-Overhead.
...
- Visuelle Matrizen-Aufgaben (non-verbales IQ-Test-Format) als möglicher nächster Content-Typ, bräuchte aber neue Bild-Assets statt nur Text/Emoji.
...
**Gemini-API-Key fehlt in der laufenden listener.ps1-Instanz (gefunden 01.08.2026, Sitzung 18)**

`.../OneDrive/2026_05_12_Lernsystem/app/listener.ps1` Zeile 32 hat aktuell den Repo-Platzhalter `HIER_GEMINI_KEY_EINTRAGEN` statt eines echten Keys — vermutlich beim letzten Deploy versehentlich überschrieben (identischer Vorfall wie in Sitzung 16, siehe dortige Doku). Per `curl` gegen `listener.ps1` bestätigt: Gemini lehnt mit „Please pass a valid API key" ab. **Betrifft alle Online-Agenten-Features gleichzeitig:** Nova-Gespräche, Fabus Live-Reaktionen, Milo-Chat, Milo-Logik-Stufe-B — alle fallen aktuell durchgehend auf ihre Offline-Fallbacks zurück (kein sichtbarer Fehler für Laetitia, aber keine „echten" dynamischen Antworten).

**Nächster Schritt (braucht den Nutzer, Claude darf keine API-Keys eintragen):**
1. Echten Key erneut von aistudio.google.com holen (Projekt „LaetitiaLernplattform")
2. In `listener.ps1` Zeile 32 eintragen
3. Listener neu starten (Task Scheduler → `Laetitia_Nova_Listener` neu auslösen, oder PC neu anmelden)
4. **Merksatz weiterhin gültig:** vor jedem künftigen Deploy von `listener.ps1` nach OneDrive prüfen, ob die Zieldatei einen echten Key enthält, der beim Überschreiben verloren ginge.
...
**Commits (alle gepusht):** `1188c7f`, `32ee5bf`, `723edc2`, `f365eb6`, `67979c5`, `9b6c054`, `cb75292`, `d161201`, `8f896fc`, `b9e98a7`, `a35aed4`, `819a27e`, `1e0ec15`

**Sitzungsabschluss:** Alle dreizehn Commits gepusht, nach jedem Schritt nach OneDrive deployed, `validate.ps1` durchgehend 0 Fehler. Git-Arbeitsverzeichnis sauber (bis auf diese Doku-Aktualisierung selbst). Browser-Tests liefen diese Sitzung größtenteils in Chrome (Erweiterung war verbunden, mit einigen bekannten Screenshot-/document_idle-Timeouts der Erweiterung selbst — kein App-Bug, siehe Muster aus früheren Sitzungen: neuer Tab statt Retry löst es zuverlässig).

**Nächste konkrete Schritte für die Folgesitzung:** Wichtigster Punkt zuerst: Gemini-API-Key in `listener.ps1` fehlt (siehe „🔴 Offene Aufgaben" oben) — sobald behoben, lohnt sich ein kurzer Live-Test von Milo-Logik-Stufe-B mit echter dynamischer Antwort. Weder Mathe- noch Logik-Modul-Ausbau haben sonst eine offene, vom Nutzer beauftragte Aufgabe — alle Vorschläge aus beiden Analysen sind abgearbeitet, inklusive der schule_mathe-Sichtbarkeit. Es bleiben nur noch zwei größere, unbeauftragte Einzelideen: Fabu-Rechengeschichten (Mathe), visuelle Matrizen-Aufgaben (Logik). Übrige offene Punkte unverändert: Fabu-Stimme-Entscheidung, Tobii-Gerätetest (Nova-Avatar), Telegram-Token, Pfad-Training-Modus-2-Bestätigung, Gruppengespräche-Implementierung, Grammatik Stufe 11+ (Thema noch offen).
